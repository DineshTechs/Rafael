...
contract LCARBON is Context, Ownable, IERC20, ERC20Detailed {
    ...
    uint256 public BUYtransactionFee = 1;
    uint256 public BUYliquidityFee = 1;
    uint256 public BUYtotalFee =
        BUYliquidityFee.add(BUYtransactionFee);

    uint256 public SELLtransactionFee = 1;
    uint256 public SELLliquidityFee = 1;
    uint256 public SELLtotalFee =
        SELLliquidityFee.add(SELLtransactionFee);

    address payable public TreasuryWallet =
        payable(0xe5C77Af24E80CF0D4e7749657ba0B776237F9B09);
    ...
    constructor() ERC20Detailed("LifeCoin Carbon", "LCARBON", 18) {
        _owner = msg.sender;
        ...
        setAutomatedMarketMakerPair(pancakeV2Pair, true);
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 walletTokens = contractTokenBalance
                .mul(SELLtransactionFee)
                .div(SELLtotalFee);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 transactionFees = newBalance.mul(SELLtransactionFee).div(
                (SELLtransactionFee)
            );

            payable(TreasuryWallet).transfer(transactionFees);
        ...
        if (sender != pancakeV2Pair && recipient != pancakeV2Pair) {
            takeFee = false;
        }
        if (takeFee) {
            if (sender == pancakeV2Pair) {
                transactionFee = BUYtransactionFee;
    ...
    function setSellFee(
        ...
        uint256 _onSellTransactionFee
    ) public onlyOwner {
        SELLtransactionFee = _onSellTransactionFee;
        SELLliquidityFee = _onSellLiquidityFee;
        uint256 onSelltotalFees;
        onSelltotalFees = SELLtransactionFee.add(
            SELLliquidityFee
        );
    ...
    function setBuyFee(
        ...
        uint256 _onBuyTransactionFee
    ) public onlyOwner {
        BUYtransactionFee = _onBuyTransactionFee;
        BUYliquidityFee = _onBuyLiquidityFee;
        uint256 onBuytotalFees;
        onBuytotalFees = BUYtransactionFee.add(BUYliquidityFee);
